...
#ifdef CONFIG_SPECTRA_CAMERA_HMCT_DEBUG
	...
	struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	s_ctrl->pipeline_delay =
		probe_info->reserved;
#ifdef CONFIG_SPECTRA_CAMERA_HMCT_DEBUG
	memcpy(s_ctrl->sensordata->slave_info.hmct_sensor_name,
		probe_info->hmct_sensor_name,sizeof(probe_info->hmct_sensor_name));
	CAM_ERR(CAM_SENSOR,"sensor name: %s", s_ctrl->sensordata->slave_info.hmct_sensor_name);
#endif
	...
	void *arg)
{
	...
	case CAM_SENSOR_PROBE_CMD: {
		...
		CAM_INFO(CAM_SENSOR,
			...
			s_ctrl->sensordata->slave_info.sensor_id);	
#ifdef CONFIG_SPECTRA_CAMERA_HMCT_DEBUG
	if(s_ctrl->sensordata->hmct_cam_pos == 0){
		productinfo_register(PRODUCTINFO_MAIN_CAMERA_ID, s_ctrl->sensordata->slave_info.hmct_sensor_name, NULL);
		CAM_ERR(CAM_SENSOR, "main camera : %s",s_ctrl->sensordata->slave_info.hmct_sensor_name);
	}else if(s_ctrl->sensordata->hmct_cam_pos ==1){
		productinfo_register(PRODUCTINFO_FRONT_CAMERA_ID, s_ctrl->sensordata->slave_info.hmct_sensor_name, NULL);
		CAM_ERR(CAM_SENSOR, "front camera : %s",s_ctrl->sensordata->slave_info.hmct_sensor_name);
	}else if(s_ctrl->sensordata->hmct_cam_pos == 2){
		productinfo_register(PRODUCTINFO_SUB_CAMERA_ID, s_ctrl->sensordata->slave_info.hmct_sensor_name, NULL);
		CAM_ERR(CAM_SENSOR, "depth camera : %s",s_ctrl->sensordata->slave_info.hmct_sensor_name);
	}else if(s_ctrl->sensordata->hmct_cam_pos == 3){
		productinfo_register(PRODUCTINFO_WIDEANGLE_CAMERA_ID, s_ctrl->sensordata->slave_info.hmct_sensor_name, NULL);
		CAM_ERR(CAM_SENSOR, "wide camera : %s",s_ctrl->sensordata->slave_info.hmct_sensor_name);
	}else if(s_ctrl->sensordata->hmct_cam_pos == 4){
		productinfo_register(PRODUCTINFO_MACRO_CAMERA_ID, s_ctrl->sensordata->slave_info.hmct_sensor_name, NULL);
		CAM_ERR(CAM_SENSOR, "micro camera : %s",s_ctrl->sensordata->slave_info.hmct_sensor_name);
	}else{
		CAM_ERR(CAM_SENSOR, "camera error productinfo");
	}
