...
#define MaximumI2CHardwareDelayinUSec   2000 //hisense add
/**
	...
	struct cam_eeprom_memory_block_t *block)
{
	...
	for (j = 0; j < block->num_map; j++) {
		...
		if (emap[j].page.valid_size) {
			...
			if(emap[j].page.delay >= MaximumI2CHardwareDelayinUSec) {  //hisense add
				i2c_reg_array.delay = emap[j].page.delay/1000;
				i2c_reg_settings.delay = emap[j].page.delay/1000;
			}
			else {
	...
	int *num_map, size_t remain_buf_len)
{
	...
	case CAMERA_SENSOR_CMD_TYPE_WAIT:
		if (generic_op_code ==
			...
			if(generic_op_code == CAMERA_SENSOR_WAIT_OP_SW_UCND)
