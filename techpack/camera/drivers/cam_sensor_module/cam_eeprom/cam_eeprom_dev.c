		...
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < otp_num; i++) {
		CAM_ERR(CAM_EEPROM, "hmct_otp_valid[%d] %s\n", i, hmct_otp_valid[i].otp_state);
		if (strlen(hmct_otp_valid[i].otp_state) > 0) {
			len = snprintf(buf + offset, PAGE_SIZE - offset, "%s;", hmct_otp_valid[i].otp_state);
			offset += len;
		}
	}
		...
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (i >= otp_num) {
		CAM_ERR(CAM_EEPROM, "hmct_otp_valid[%d] overflow.\n", i);
		return -EFAULT;
	}
	...
	struct platform_device *pdev)
{
	...
	if (otp_syfs_created == 0) {
		if (his_register_sysfs_attr(&dev_attr_hs_cam_otp.attr) < 0 ) {
			pr_err("Error creating eeprom_dbg_node\n");
		}
		otp_syfs_created = 1;
	}
#endif
