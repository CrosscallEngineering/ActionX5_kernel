...
#ifdef CONFIG_KTD3133_BL
extern void ktd3137_brightness(int brightness);
extern int ktd3137_update_brightness_for_lcd_use(int brightness);
#endif

#ifdef CONFIG_KTZ8866_BL
...
{
	int rc = 0;
	DSI_WARN("[%s]",__FUNCTION__);
#ifdef CONFIG_KTD3133_BL
	ktd3137_brightness(1);
#endif
	...
	if (gpio_is_valid(panel->vddio_en_gpio)) {
		...
		if (rc) {
			DSI_ERR("unable to set dir for vddio_en_gpio rc=%d\n", rc);
			goto exit;
		}
	}
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	case DSI_BACKLIGHT_EXTERNAL:
		#ifdef CONFIG_KTD3133_BL
		ktd3137_update_brightness_for_lcd_use(bl_lvl);
		#endif
		#ifdef CONFIG_KTZ8866_BL
...
static int dsi_panel_parse_misc_features(struct dsi_panel *panel)
{
	...
	panel->power_sequence = utils->read_bool(utils->data,
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	if (!gpio_is_valid(panel->reset_config.reset_gpio) &&
		...
		DSI_WARN("[%s] failed get reset gpio, rc=%d\n", panel->name, rc);
		//goto error;
	}
	...
	if (!gpio_is_valid(panel->vddio_en_gpio))
       {
		DSI_DEBUG("[%s] failed get vddio en gpio, rc=%d\n", panel->name, rc);
	}	
