...
void sensors_report_1(struct input_dev *idev, int val_1)
{
	...
	if (val_1 < 2) {
		...
		switch (val_1) {
		case 2:
			input_report_abs(idev, ABS_DISTANCE, -99);
			input_report_abs(idev, ABS_DISTANCE, -100);
			break;
		case 3:
			input_report_abs(idev, ABS_DISTANCE, -199);
			input_report_abs(idev, ABS_DISTANCE, -200);
			break;
		//default:
		}
	}
		...
		char *name, int axes_num, void *priv_data)
{
	...
	if (!i_dev) {
		...
		switch (axes_num) {
		case 1:
			input_set_abs_params(i_dev, ABS_DISTANCE, -65535, 65535, 0, 0);
			break;
		case 2:
