...
static int mmc_decode_cid(struct mmc_card *card)
{
	...
	if(!card->host->index)
    {
		...
		else if(card->cid.manfid==CID_MANFID_NUMONYX)
...
static void mmc_add_emmcinfo_to_productinfo(struct mmc_card *card, u8 *ext_csd)
{
	...
	if (card ) {
		if (!strcmp(mmc_hostname(card->host), "mmc0")){
			if( MMC_BUS_WIDTH_8 != card->host->ios.bus_width){
				pr_buf_err("%s:eMMC bus 8 bits width set fail, error: %d\n", __func__, err);
			}
		}
	}
	return err;
