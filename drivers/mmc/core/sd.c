...
static int _mmc_sd_resume(struct mmc_host *host)
{
	...
	if (err == -ENOENT) {
		pr_buf_err("%s: %s: found a different card(%d), do detect change\n",
			mmc_hostname(host), __func__, err);
		mmc_card_set_removed(host->card);
...
err:
	...
	if(host->ops->get_cd){
		...
		if(0 == present || -ENOMEDIUM == err){
			pr_err("%s: %s err: SD card plugged out or cd=%d\n", mmc_hostname(host), __func__, present);
			sd_err_status_report(virtual_T_disk, SD_PLUG_OUT);
		}
		else if(-ENOSYS != present){
