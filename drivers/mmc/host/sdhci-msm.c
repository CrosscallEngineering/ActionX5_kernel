...
out:
	...
	if(mmc_host_sd(host->mmc)) {
		reg = readl_relaxed(host->ioaddr + msm_host_offset->CORE_VENDOR_SPEC);
		reg &= ~CORE_CLK_PWRSAVE;
...
static void sdhci_msm_set_clock(struct sdhci_host *host, unsigned int clock)
{
    ...
    if(mmc_host_sd(host->mmc) && curr_pwrsave)
    {
...
#define BOOT_FROM_SDHCI  "sdhci"
#define BOOT_FROM_UFS    "ufs"

static int get_boot_storage_type(char **device)
{
    char boot[20] = {'\0'};
    char *match = (char *)strnstr(saved_command_line, "androidboot.bootdevice=",\
            strlen(saved_command_line));

    if (match) {
        memcpy(boot, (match + strlen("androidboot.bootdevice=")), sizeof(boot) - 1);
        if (strnstr(boot, "sdhci", strlen(boot)))
            *device = BOOT_FROM_SDHCI;
        else if (strnstr(boot, "ufs", strlen(boot)))
            *device = BOOT_FROM_UFS;

        pr_debug("%s, %d: boot device is %s!\n", __func__, __LINE__, *device);
        return 0;
    }

    pr_err("%s, %d: No valid androidboot.bootdevice= info!\n", __func__, __LINE__);
    return -EINVAL;
}

bool mmc_host_sd(struct mmc_host *mmc)
{
    char *boot = "unknown";
    bool ret = false;
    ...
    if(get_boot_storage_type(&boot) < 0)
    {
        return false;
    }

    if(!strncmp(boot, BOOT_FROM_SDHCI, sizeof(BOOT_FROM_SDHCI)) && \
            !strncmp(mmc_hostname(mmc), "mmc1", sizeof("mmc1")))
    {
        dev_dbg(mmc_dev(mmc), "%s: mmc1 host is sd.\n",__func__);
        ret = true;
    }
    else if(!strncmp(boot, BOOT_FROM_UFS, sizeof(BOOT_FROM_UFS)) && \
            !strncmp(mmc_hostname(mmc), "mmc0", sizeof("mmc0")))
    {
        dev_dbg(mmc_dev(mmc), "%s: mmc0 host is sd.\n",__func__);
        ret = true;
    }
