...
int smb5_configure_typec(struct smb_charger *chg);
#endif
...
static int smb5_parse_dt_misc(struct smb5 *chip, struct device_node *node)
{
	...
	rc = of_get_named_gpio(node, "qcom,otg-id-gpio", 0);
	chg->otg_id_gpio = rc;
	if(!gpio_is_valid(chg->otg_id_gpio)) {
		dev_err(chg->dev, "invalid otg_id_gpio rc = %d\n", rc);
	}
	rc = gpio_request(chg->otg_id_gpio, "otg-id-gpio");
	if (rc < 0) {
		dev_err(chg->dev, "%s: gpio %d request failed. rc = %d\n",
			__func__, chg->otg_id_gpio, rc);
	}

#endif /*CONFIG_HISENSE_GPIO_TRIG_OTG*CONFIG_HISENSE_TYPEC_POGO_PEN*/

#ifdef CONFIG_HISENSE_TYPEC_POGO_PEN
	rc = of_get_named_gpio(node, "qcom,pogo-out-en", 0);
	chg->pogo_out_en = rc;
	if(!gpio_is_valid(chg->pogo_out_en)) {
		dev_err(chg->dev, "invalid pogo-out-en rc = %d\n", rc);
	}
	rc =gpio_request(chg->pogo_out_en,"pogo-out-en");
	if(rc < 0){
		dev_err(chg->dev,"gpio %d request failed, ret = %d\n",chg->pogo_out_en,rc);
	}
	rc = of_get_named_gpio(node, "qcom,pogo-usb-switch", 0);
	chg->pogo_usb_switch = rc;
	if(!gpio_is_valid(chg->pogo_usb_switch)) {
		dev_err(chg->dev, "invalid pogo usb switch rc = %d\n", rc);
	}
	rc =gpio_request(chg->pogo_usb_switch,"pogo-usb-switch");
	if(rc < 0){
		dev_err(chg->dev,"gpio %d request failed, ret = %d\n",chg->pogo_usb_switch,rc);
	}
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGER_REDET:
		...
		if (rc < 0)
			pr_err("Couldn't un-vote for USB ICL rc=%d\n", rc);
...
int smb5_configure_typec(struct smb_charger *chg)
{
	...
	rc = smblib_masked_write(chg, TYPEC_U_USB_CFG_REG,
				EN_MICRO_USB_MODE_BIT | EN_MICRO_USB_FACTORY_MODE_BIT, 0);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't enable typec usb mode c=%d\n", rc);
		return rc;
	}

	chg->connector_type = POWER_SUPPLY_CONNECTOR_TYPEC;
	pr_buf_info("%s: connector type is typec done.\n", __func__);
#endif /* CONFIG_HISENSE_SGM2540 */
	return rc;
}

int smb5_configure_micro_usb(struct smb_charger *chg)
{
	int rc;
	...
	rc = smblib_masked_write(chg, TYPEC_U_USB_CFG_REG,
				...
				EN_MICRO_USB_MODE_BIT);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't enable micro usb mode c=%d\n", rc);
		return rc;
	}

	chg->connector_type = POWER_SUPPLY_CONNECTOR_MICRO_USB;
	pr_buf_info("%s: connector type is micro done.\n", __func__);
#endif /* CONFIG_HISENSE_SGM2540 */
...
static int smb5_init_connector_type(struct smb_charger *chg)
{
	...
	pr_buf_info("%s: Connector type=%s\n", __func__, type ? "Micro USB" : "TypeC");
...
void smb5_configure_afp_enable(bool enable)
{
	...
	if (global_smb5_chip) {
		rc = smblib_masked_write(&global_smb5_chip->chg, JEITA_EN_CFG_REG,
			mask, val);
		if (rc < 0)
			pr_buf_err("%s: %s AFP failed rc=%d\n", __func__, enable ? "enable" : "disable", rc);
	}

	pr_buf_err("%s: %s AFP done. rc=%d.\n", __func__, enable ? "enable" : "disable", rc);
...
#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION
	/*Set default aicl threshold to 4v*/
	chg->default_aicl_5v_threshold_mv = 4000;
...
static int smb5_post_init(struct smb5 *chip)
{
	...
	if (dev_bi.bootmode == BOOT_FACTORY_MODE) {
		pr_buf_err("%s: ftm mode, otg default enable\n", __func__);
		chg->disable_otg = false;
	} else {
		pr_buf_err("%s: otg default disable\n", __func__);
		chg->disable_otg = true;
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!global_smb5_chip) {
		...
		if (global_smb5_chip->chg.connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB)
			val.intval = 0;
		else
			smblib_get_prop_typec_cc_orientation(&global_smb5_chip->chg, &val);
	}
...
static ssize_t fast_charge_status_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	pr_buf_info("@@@@%s: fastcharge_flags is %d\n", __func__, global_smb5_chip->chg.fastcharge_flags);
	return snprintf(buf, PAGE_SIZE, "%d\n",  !!global_smb5_chip->chg.fastcharge_flags);
}

static DEVICE_ATTR(fastcharge, S_IRUGO, fast_charge_status_show, NULL);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	disable = !!state;
	pr_buf_err("%s: %sable otg %d\n", __func__, disable ? "dis" : "en", chg->disable_otg);
	if (disable != chg->disable_otg) {
		chg->disable_otg = disable;
		if (chg->typec_mode == POWER_SUPPLY_TYPEC_SINK) {
			pr_buf_err("%s POWER_SUPPLY_TYPEC_SINK mode %s vbus\n", __func__, chg->disable_otg ? "disable" : "enable");
			smblib_notify_usb_host(chg, !chg->disable_otg);
			/* PMI632 can not support pd, chg->vbus_vreg is uninitialized, so use smblib_notify_usb_host() to enable/disable otg.
			...
			pr_buf_err("%s source is present,do nothing\n", __func__);
		} else {
			pval.intval = POWER_SUPPLY_TYPEC_PR_DUAL;
			rc = smblib_set_prop_typec_power_role(chg, &pval);
			if (rc < 0) {
...
static DEVICE_ATTR_RW(type_c_disable_otg);

static ssize_t host_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", !!g_chip->chg.otg_present);
}

static DEVICE_ATTR_RO(host_mode);
#endif /*CONFIG_HISENSE_TYPEC_OTG_CONTROL_FUNCTION*/


#ifdef CONFIG_HISENSE_TYPEC_POGO_PEN
static struct smb5 *g_chip_otg = NULL;
enum otgtypemode
{
	POGO_DEFAULT_MODE = 0,
	USB_OTG_MODE=5,
...
static ssize_t usb_otg_type_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct smb_charger *chg = &(g_chip_otg->chg);
	int mode = POGO_DEFAULT_MODE;
	bool pogo_otg = !gpio_get_value(chg->otg_id_gpio);
	if(g_chip_otg->chg.otg_present){
		mode = USB_OTG_MODE;
		if(pogo_otg){
			mode = POGO_OTG_MODE;
		}
		pr_buf_err("curent_mode=0x%x, pogo_otg %d\n",mode, pogo_otg);
	}
	if(pogo_otg){
		mode = POGO_OTG_MODE;
	}
	pr_buf_info("curent_mode=0x%x, otg_present %d,pogo_otg\n",mode,g_chip_otg->chg.otg_present,pogo_otg);
	return sprintf(buf, "%d\n", mode);
}

static DEVICE_ATTR_RO(usb_otg_type);
static void usb_otg_type_init(struct smb5 *chip)
{
	int ret =0;
	g_chip_otg = chip;
	ret = his_register_sysfs_attr(&dev_attr_usb_otg_type.attr);
	if (ret < 0)
			pr_buf_err("Couldn't create usb_otg_type file ret=%d\n", ret);
}
#endif /*CONFIG_HISENSE_TYPEC_POGO_PEN*/
#ifdef CONFIG_HISENSE_UKL_SAFE_MODE_FUNCTION
static struct smb5 *g_uchip = NULL;
static ssize_t ukl_safe_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", g_uchip->chg.enable_ukl_safe_mode);
}

static ssize_t ukl_safe_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	union power_supply_propval pval = {0, };
	bool benable = true;
	int rc = 0;
	unsigned long state = 0;
	struct smb_charger *chg = &(g_uchip->chg);
	...
	benable = !!state;
	pr_buf_err("%s: %sable ukl_safe_mode %d\n", __func__, benable ? "en" : "dis", chg->enable_ukl_safe_mode);
	if (benable != chg->enable_ukl_safe_mode) {
		chg->enable_ukl_safe_mode = benable;
		if(chg->enable_ukl_safe_mode){
			pr_buf_err("%s: %s enter ukl safe mode\n", __func__, benable ? "en" : "dis");
			if (chg->fv_votable)
			{
				vote(chg->fv_votable, UKL_SAFE_VOTER, true , UKL_SAFE_VOL_UV);
				pr_buf_err("%s: %s, %s set float voltage %d\n", __func__,UKL_SAFE_VOTER, benable ? "en" : "dis", UKL_SAFE_VOL_UV);
				pval.intval = 1;
				rc = power_supply_set_property(chg->batt_psy,POWER_SUPPLY_PROP_INPUT_SUSPEND, &pval);
				if (rc < 0) {
					pr_err("Couldn't change slave suspend state rc=%d\n",rc);
				}
				pr_buf_err("%s: %s suspend input..\n", __func__, benable ? "en" : "dis" );		
			}
		}else{
				pr_buf_err("%s: %s recover the fv voltage and enable charging after exit ukl safe mode..\n", __func__, benable ? "en" : "dis");
...
static DEVICE_ATTR_RW(ukl_safe_mode);
...
#ifdef CONFIG_HISENSE_TYPEC_OTG_CONTROL_FUNCTION
	if (chip->chg.connector_type == POWER_SUPPLY_CONNECTOR_TYPEC){
		g_chip = chip;
		ret = his_register_sysfs_attr(&dev_attr_type_c_disable_otg.attr);
		if (ret < 0)
			pr_buf_err("Couldn't create typec_cc_disable_otg file ret=%d\n", ret);
		ret = his_register_sysfs_attr(&dev_attr_host_mode.attr);
		if (ret < 0)
			pr_buf_err("Couldn't create type_c_host_mode_show file ret=%d\n", ret);
	}
#endif /* CONFIG_HISENSE_TYPEC_OTG_CONTROL_FUNCTION */
#ifdef CONFIG_HISENSE_TYPEC_POGO_PEN
	usb_otg_type_init(chip);
#endif /*CONFIG_HISENSE_TYPEC_POGO_PEN*/
#ifdef CONFIG_HISENSE_UKL_SAFE_MODE_FUNCTION
		g_uchip = chip;
		ret = his_register_sysfs_attr(&dev_attr_ukl_safe_mode.attr);
		if (ret < 0)
			pr_buf_err("Couldn't create ukl_safe_mode file ret=%d\n", ret);
#endif /*CONFIG_HISENSE_UKL_SAFE_MODE_FUNCTION*/
...
static int smb5_show_charger_status(struct smb5 *chip)
{
	...
	pr_buf_info("SMB5 status - usb:present=%d type=%d batt:present = %d health = %d charge = %d\n",
