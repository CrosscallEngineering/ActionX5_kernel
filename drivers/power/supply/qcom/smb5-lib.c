...
extern bool sgm2540_bf_online(void);
extern bool sgm2540_af_gpio_status(void);
extern int sgm2540_a_otg_vbus_enable(bool enable);
extern int smb5_configure_typec(struct smb_charger *chg);
extern void sgm2540_usb_switch(bool enable);
extern bool sgm2540_af_charger_ok(void);
#endif /*CONFIG_HISENSE_SGM2540*/

#ifdef CONFIG_HISENSE_SGM41512_CHARGER
extern bool sgm_ext_otg_ok(void);
extern bool sgm_typec_otg_ok(void);
...
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
{
	...
	if (enable) {
#ifdef CONFIG_HISENSE_SGM41512_CHARGER
		if (!sgm_ext_otg_ok() && !sgm_typec_otg_ok()) {
...
#ifdef CONFIG_HISENSE_SGM2540
	if (sgm2540_af_charger_ok() &&
		(apsd_result->pst == POWER_SUPPLY_TYPE_USB_FLOAT || apsd_result->pst == POWER_SUPPLY_TYPE_USB_DCP)) {
		sgm2540_usb_switch(false);
	}
#endif /*CONFIG_HISENSE_SGM2540*/
...
#ifdef CONFIG_HISENSE_SGM2540
	if (!sgm2540_af_gpio_status() &&
		chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB) {
		smb5_configure_typec(chg);
	}
#endif /*CONFIG_HISENSE_SGM2540*/
				  ...
				  union power_supply_propval *val)
{
	...
	pr_buf_info("@@@@%s intval=0x%x, val->intval= %d\n", __func__, value, val->intval);
	return 0;
}
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
			       ...
			       union power_supply_propval *val)
{
	int rc = 0;
	u8 stat;
#ifdef CONFIG_HISENSE_UKL_SAFE_MODE_FUNCTION
	union power_supply_propval pval = {0, };
	int suspend =0,vbus_mv = 0;
#endif/*CONFIG_HISENSE_UKL_SAFE_MODE_FUNCTION*/
	...
	if(chg->enable_ukl_safe_mode){
		...
		smblib_err(chg, "check input is suspend or not: suspend %d, vbus %d mv\n",suspend,vbus_mv);
		if((suspend) && (vbus_mv >= 3600)){
			val->intval = true;
			return rc;
		}
	}
    ...
    if(!val->intval && jeita_bat_ov) {
	    pr_buf_info("%s, USB not online && jeita battery ov\n", __func__);
	    if((stat & USE_USBIN_BIT) && (chg->real_charger_type!=POWER_SUPPLY_TYPE_UNKNOWN)) {
            pr_buf_info("%s, USB is plugged actually, force usb online\n", __func__);
            val->intval = 1;
	    }
    }
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
					...
					union power_supply_propval *val)
{
	...
	if (sgm_mag_charger_ok()) {
		...
		power_supply_get_property(chg->sgm_chg_psy, POWER_SUPPLY_PROP_VOLTAGE_MAX, &pval);
		val->intval = pval.intval;
		return 0;
	}
#endif /*CONFIG_HISENSE_SGM41512_CHARGER*/
...
static int smblib_get_prop_ufp_mode(struct smb_charger *chg)
{
	...
	if (stat & (SNK_DAM_500MA_BIT | SNK_DAM_1500MA_BIT | SNK_DAM_3000MA_BIT)) {
		smblib_masked_write(chg, TYPE_C_DEBUG_ACCESS_SINK_REG, TYPE_C_DEBUG_ACCESS_SINK_MASK, 0x17);
		pr_buf_info("%s: DAM cable found, will set usb typec source to defualt.\n", __func__);
	}
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
					  ...
					  union power_supply_propval *val)
{
	...
	if (sgm_mag_charger_ok()) {
		...
		power_supply_get_property(chg->sgm_chg_psy, POWER_SUPPLY_PROP_CURRENT_MAX, &pval);
		val->intval = pval.intval;
		return 0;
	}
#endif /*CONFIG_HISENSE_SGM41512_CHARGER*/
					...
					int usb_current)
{
	...
	pr_buf_err("%s real_charger_type %d, usb_current %d mA\n", __func__, chg->real_charger_type, usb_current);
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
...
#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION
		/*Solving abnormal current setting of float charger after system bringup.*/
		if (apsd_result->pst == POWER_SUPPLY_TYPE_USB_FLOAT  && (chg->float_icl_ua > 0))
			usb_current = chg->float_icl_ua;
		pr_buf_info("%s APSD result is FLOAT, so change the usb_current %d mA\n", __func__, usb_current);
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
...
#ifdef CONFIG_HISENSE_TYPEC_OTG_CONTROL_FUNCTION
		if (chg->disable_otg)
			power_role = EN_SNK_ONLY_BIT;
#endif /* CONFIG_HISENSE_TYPEC_OTG_CONTROL_FUNCTION */
		break;
...
extern int ft_ctp_work_with_ac_usb_plugin(int plugin);
#endif /*CONFIG_TOUCHSCREEN_FT5X06_FH*/
extern int ts_get_usb_plugin_out_status(int plugin);
static void smb_ctp_work(struct work_struct *work)
{
#if defined(CONFIG_TOUCHSCREEN_FT5X06_FH)
	ft_ctp_work_with_ac_usb_plugin((usb_flag ? 1 : 0));
#endif/*CONFIG_TOUCHSCREEN_FT5X06_FH*/
	ts_get_usb_plugin_out_status((usb_flag ? 1 : 0));
}
#endif /* CONFIG_TOUCHSCREEN_USB_CHECK */
					    ...
					    bool rising)
{
	...
	if (rising && (chg->fastcharge_flags < 3)) {
		kobject_uevent_env(&chg->dev->kobj, KOBJ_CHANGE, quick_charger);
		chg->fastcharge_flags++;
	}
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
...
static void update_sw_icl_max(struct smb_charger *chg, int pst)
{
	...
	case POWER_SUPPLY_TYPE_USB_FLOAT:
		 ...
		 */
#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION
		pr_buf_info("chg->float_icl_ua=%d\n",chg->float_icl_ua);
		vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true,
					chg->float_icl_ua);
#else
		vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true,
...
irqreturn_t usb_source_change_irq_handler(int irq, void *data)
{
	...
	if (chg->real_charger_type != POWER_SUPPLY_TYPE_USB_DCP && uusb_recheck_count < USB_RECHECK_COUNT) {
		schedule_delayed_work(&chg->uusb_recheck_work, msecs_to_jiffies(5000));
	}
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
	rc = smblib_read(chg, APSD_STATUS_REG, &stat);
	if (rc < 0) {
...
static void smblib_initial_recheck_otg_work(struct work_struct *work)
{
	...
	pr_buf_err("%s: typec_cc_orientation=%d, otg_present=%d sink_src_mode=%d\n",
		__func__, prop_val.intval, chg->otg_present,  chg->sink_src_mode);
	/*Typec cc attached, but sink_src_mode is unattached, we need recheck status.*/
	if ((prop_val.intval != 0) && chg->sink_src_mode == UNATTACHED_MODE)
...
static void smblib_uusb_recheck_work(struct work_struct *work)
{
	...
	vbus_rising = (bool)(stat & USBIN_PLUGIN_RT_STS_BIT);
	pr_buf_err("%s:real_charger_type=%d,connection=%d\n", __func__, chg->real_charger_type, connection);
	if ((chg->real_charger_type == POWER_SUPPLY_TYPE_USB) && (connection == false) && vbus_rising) {
			...
			if ((chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB)
				&& (stat & APSD_DTC_STATUS_DONE_BIT)) {
				smblib_rerun_apsd_if_required(chg);
				pr_buf_info("%s:rerun apsd\n", __func__);
			}
			uusb_recheck_count++;
			pr_buf_err("%s:uusb_recheck_count = %d\n", __func__, uusb_recheck_count);
	}
}
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
...
#ifdef CONFIG_HISENSE_SGM41512_CHARGER
	if (sgm_ext_otg_ok()) {
		pr_err("###sgm ext otg ok,typec otg do noting\n");
		return;
	}
#endif /*CONFIG_HISENSE_SGM41512_CHARGER*/
...
static void typec_src_removal(struct smb_charger *chg)
{
	...
	pval.intval = 0;
	if(chg->enable_ukl_safe_mode){
		pr_err("%s: clean enable_ukl_safe_mode flags.\n", __func__);
		/*usb was remove, so clear the enable_ukl_safe_mode flag*/
		chg->enable_ukl_safe_mode = false;
	...
	connection = false;
	uusb_recheck_count = 0;
	chg->fastcharge_flags = 0;
	pr_buf_info("%s: clean hmct-charge flags.\n", __func__);
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
...
#ifdef CONFIG_HISENSE_UKL_SAFE_MODE_FUNCTION
	/*when typec removal need recover FV voltage to 435V(max battery voltage)*/
	vote(chg->fv_votable, UKL_SAFE_VOTER, false, 0);
#endif /*CONFIG_HISENSE_UKL_SAFE_MODE_FUNCTION*/
...
irqreturn_t wdog_snarl_irq_handler(int irq, void *data)
{
	...
	struct smb_charger *chg = irq_data->parent_data;
#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION
	struct battery_prop_cache cache = {0, };
	union power_supply_propval pval = {0, };
	int rc = 0;
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
	...
	rc = smblib_get_prop_batt_capacity(chg, &pval);
	if (rc < 0)
	    smblib_err(chg, "Couldn't get battery capacity rc=%d\n", rc);
	else
	    cache.capacity = pval.intval;

	rc = smblib_get_prop_batt_health(chg, &pval);
	if (rc < 0)
	    smblib_err(chg, "Couldn't get battery health rc=%d\n", rc);
	else
	    cache.health = pval.intval;

	rc = smblib_get_prop_batt_status(chg, &pval);
	if (rc < 0)
	    smblib_err(chg, "Couldn't get battery status rc=%d\n", rc);
	else
	    cache.status = pval.intval;

	smblib_dbg(chg, PR_INTERRUPT, "battery-capacity:%d, health:%d, status:%d\n",
	        cache.capacity, cache.health, cache.status);

	if (memcmp(&chg->battery_cache, &cache, sizeof(cache)) != 0) {
	    chg->battery_cache = cache;
	    power_supply_changed(chg->batt_psy);
	}
#else
	power_supply_changed(chg->batt_psy);
...
#ifdef CONFIG_HISENSE_TYPEC_POGO_PEN
static int pogo_pen_otg_vbus_enable(struct smb_charger *chg, bool enable)
{
	int rc = 1;

	if(gpio_is_valid(chg->pogo_out_en)) {
		rc = gpio_direction_output(chg->pogo_out_en, enable);
		if (rc)
			dev_err(chg->dev,"unable to set dir for pogo_out_en rc=%d\n", rc);
	}
	if(gpio_is_valid(chg->pogo_usb_switch)) {
		rc = gpio_direction_output(chg->pogo_usb_switch, enable);
		if (rc)
			dev_err(chg->dev,"unable to set dir for pogo_usb_switch rc=%d\n", rc);
	}

	pr_buf_info("%s: pogo pen otg vbus channel %s\n",enable?"enable":"disable", !rc?"success":"fail");
	return rc;
}
static irqreturn_t smblib_pogo_pen_irq_handler(int irq, void *data)
{
	struct smb_charger *chg = data;

	//pr_buf_info("@@@%s, chg->connector_type is %d\n", __func__, chg->connector_type);
	cancel_delayed_work_sync(&chg->usbid_otg_work);
	schedule_delayed_work(&chg->usbid_otg_work,
		msecs_to_jiffies(chg->otg_delay_ms));
	return IRQ_HANDLED;
}
#endif /*CONFIG_HISENSE_TYPEC_POGO_PEN*/

#if defined(CONFIG_HISENSE_GPIO_TRIG_OTG) ||defined(CONFIG_HISENSE_TYPEC_POGO_PEN)
static void smblib_usbid_otg_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
						usbid_otg_work.work);
	int rc;
	bool usbid_otg = !gpio_get_value(chg->otg_id_gpio);

	vote(chg->awake_votable, OTG_DELAY_VOTER, true, 0);

	pr_buf_err("%s: last usbid_otg = %d, now usbid_otg = %d\n", __func__, chg->usbid_otg_present, usbid_otg);

	if (chg->usbid_otg_present != usbid_otg){
#ifdef CONFIG_HISENSE_SGM2540
		/*Remove disable magconn otg into dwc3_otg_start_host()
		 * to solve af irq trigger because of vbus not draw down.
		 */
		if (usbid_otg)
			sgm2540_a_otg_vbus_enable(usbid_otg);
#endif /*CONFIG_HISENSE_SGM2540*/
#ifdef CONFIG_HISENSE_TYPEC_POGO_PEN
		pogo_pen_otg_vbus_enable(chg,usbid_otg);
#endif /*CONFIG_HISENSE_TYPEC_POGO_PEN*/
		smblib_notify_usb_host(chg, usbid_otg);
	}
	else
		goto out;

	chg->usbid_otg_present = usbid_otg;
	if (!usbid_otg)
		chg->boost_current_ua = 0;

	rc = smblib_set_charge_param(chg, &chg->param.freq_switcher,
				usbid_otg ? chg->chg_freq.freq_below_otg_threshold
					: chg->chg_freq.freq_removal);
	if (rc < 0)
		dev_err(chg->dev, "Error in setting freq_boost rc=%d\n", rc);
...
#ifdef	CONFIG_HISENSE_GPIO_TRIG_OTG
static irqreturn_t smblib_usbid_irq_handler(int irq, void *data)
{
	struct smb_charger *chg = data;
...
#ifdef CONFIG_HISENSE_SGM2540
	if (!sgm2540_bf_online()) {
#else /*CONFIG_HISENSE_SGM2540*/
	if (chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB)
	{
#endif /*CONFIG_HISENSE_SGM2540*/
		cancel_delayed_work_sync(&chg->usbid_otg_work);
...
#endif	/*CONFIG_HISENSE_GPIO_TRIG_OTG*/

static int smblib_usbid_irq_init(struct smb_charger *chg)
{
	...
	if(!gpio_is_valid(chg->otg_id_gpio)) {
		dev_err(chg->dev, "%s: invalid otg_id_gpio, init return\n", __func__);
		return ret;
	}

	chg->otg_id_irq = gpio_to_irq(chg->otg_id_gpio);
#ifdef CONFIG_HISENSE_TYPEC_POGO_PEN
	ret = request_threaded_irq(chg->otg_id_irq ,
			NULL,
			smblib_pogo_pen_irq_handler,
			IRQF_TRIGGER_RISING |
	...
	ret = request_threaded_irq(chg->otg_id_irq ,
			NULL,
			smblib_usbid_irq_handler,
			IRQF_TRIGGER_RISING |
			IRQF_TRIGGER_FALLING |IRQF_ONESHOT,
			"usbid-otg", chg);
#endif
	if (ret) {
		pr_buf_err("request irq failed for usbid irq\n");

	}

	/*If usb otg is attached before init, we should schedule work to mount otg.*/
	if (usbid_otg)
		schedule_delayed_work(&chg->usbid_otg_work,
			msecs_to_jiffies(chg->otg_delay_ms));

	enable_irq_wake(chg->otg_id_irq);
	pr_buf_info("%s: done\n", __func__);

	return ret;

}
#endif /*CONFIG_HISENSE_GPIO_TRIG_OTG*CONFIG_HISENSE_TYPEC_POGO_PEN*/
...
int smblib_init(struct smb_charger *chg)
{
...
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
#if defined(CONFIG_HISENSE_GPIO_TRIG_OTG) ||defined(CONFIG_HISENSE_TYPEC_POGO_PEN)
	INIT_DELAYED_WORK(&chg->usbid_otg_work, smblib_usbid_otg_work);
#endif /*CONFIG_HISENSE_GPIO_TRIG_OTG*CONFIG_HISENSE_TYPEC_POGO_PEN*/
	...
	memset(&chg->battery_cache, 0, sizeof(struct battery_prop_cache));
#endif /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
...
#endif /* CONFIG_TOUCHSCREEN_USB_CHECK */
#if defined(CONFIG_HISENSE_GPIO_TRIG_OTG) ||defined(CONFIG_HISENSE_TYPEC_POGO_PEN)
	rc = smblib_usbid_irq_init(chg);
	if (rc) {
		smblib_err(chg, "usbid irq init failed\n");
	}
#endif /*CONFIG_HISENSE_GPIO_TRIG_OTG*CONFIG_HISENSE_TYPEC_POGO_PEN*/
