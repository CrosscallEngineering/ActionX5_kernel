...
static void ttf_work(struct work_struct *work)
{
	...
	int batt_temp = 0, rbatt = 0, empty_volt_mv = 0,vbus = 0,charge_type = 0;
	...
	pr_buf_info("TTF: charge_status:%d charge_done:%d msoc:%d\n",
	...
	if (rc < 0) {
		pr_buf_err("Error in getting temp, rc=%d\n", rc);
	}
	rc = ttf->get_ttf_param(ttf->data, TTF_RBATT, &rbatt);
	if (rc < 0) {
		pr_buf_err("Error in getting rbatt, rc=%d\n", rc);
	}
	rc = ttf->get_ttf_param(ttf->data, TTF_DEAD_MV, &empty_volt_mv);
	if (rc < 0) {
		pr_buf_err("Error in getting empty_mv, rc=%d\n", rc);
	}
	pr_buf_err("vbus=%d, charge_type=%d, ibatt_now=%d, vbatt_now=%d, capacity=%d, temp=%d, charge_status=%d, rbatt=%d\n",
	...
	if (vbatt_now <= (empty_volt_mv * 1000)) {
		...
		vbat_dead_count++;
		pr_buf_err("bat is becoming dead %d count %d\n",vbatt_now, vbat_dead_count);
