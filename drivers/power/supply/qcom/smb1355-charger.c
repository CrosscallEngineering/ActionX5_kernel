...
#define pr_fmt(fmt) "--PM-SMB1355: %s: " fmt, __func__
...
static int smb1355_set_parallel_charging(struct smb1355 *chip, bool disable)
{
	...
	pr_buf_info("%s:configure disable=%d\n", __func__, disable);
...
static int smb1355_detect_version(struct smb1355 *chip)
{
	...
	do {
		...
		if (rc < 0) {
			i++;
			pr_err("Unable to read REVID rc = %d, i = %d\n", rc, i);
		} else
			break;
	} while (rc < 0 && (i <= 10));
	...
	if (rc < 0 && (i >= 10)) {
		...
		pr_err("Forced set REVID to smb1355,  rc = %d, i = %d\n", rc, i);
		return 0;
	}
