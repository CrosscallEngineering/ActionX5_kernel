...
#ifdef CONFIG_HISENSE_UKL_SAFE_MODE_FUNCTION
static int qg_get_near_cap_from_vbat(int vbat_uv)
{
	int left_index, right_index,mid_index;
	int itermCap = 0;

	left_index = 0;
	right_index = MAX_BAT_CAP_LENGTH - 1;
	mid_index = 0;

	if(vbat_uv >= bat_cap_array[left_index].vbat_uv){
		itermCap = bat_cap_array[left_index].capacitymAh;
		pr_err("Warning: vbat %d is above bat_cap_array max value %d , cap %d mV",vbat_uv, bat_cap_array[left_index].vbat_uv,bat_cap_array[left_index].capacitymAh);
		return itermCap;
	}
	if(vbat_uv <= bat_cap_array[right_index].vbat_uv){
		itermCap = bat_cap_array[right_index].capacitymAh;
		pr_err("Warning: vbat %d is below bat_cap_array min value %d , cap %d mV",vbat_uv, bat_cap_array[right_index].vbat_uv,bat_cap_array[right_index].capacitymAh);
		return itermCap;
	}

	while(left_index != right_index){
		mid_index = (right_index + left_index)/2;
		if(vbat_uv <= bat_cap_array[mid_index].vbat_uv){
	...
	itermCap = qg_linear_interpolate(bat_cap_array[right_index].capacitymAh, (bat_cap_array[right_index].vbat_uv/1000),\
						bat_cap_array[left_index].capacitymAh, (bat_cap_array[left_index].vbat_uv/1000), \
						(vbat_uv/1000));
	pr_err("vbat_uv %d uV, linearCap %d mAh, Left [%d] %d uV, Cap %d mAh, Right[%d]  %d uV, Cap %d mAh",vbat_uv,itermCap,\
		left_index, bat_cap_array[left_index].vbat_uv, bat_cap_array[left_index].capacitymAh,\
		right_index, bat_cap_array[right_index].vbat_uv,bat_cap_array[right_index].capacitymAh);

	return itermCap;
}
static void qg_check_iterm_capacity(struct qpnp_qg *chip)
{
	union power_supply_propval prop = {0, };
	int rc = 0, iterm_fv_uv = 0,effective_fv_uv =0;
	int iterm_cap_mAh = 0;
	...
	if (rc < 0)
		pr_err("Failed to get vbatt, rc=%d\n", rc);

	pr_err("Charge done iterm , voltage %d mV",prop.intval/1000);
	iterm_fv_uv = prop.intval;

	rc = power_supply_get_property(chip->batt_psy,POWER_SUPPLY_PROP_VOLTAGE_MAX, &prop);
	if (rc < 0)
		pr_err("Failed to ge max voltage uv, rc=%d\n", rc);

	effective_fv_uv = prop.intval;

	if (iterm_fv_uv >= effective_fv_uv + 40000) {
		pr_err("battery over-voltage vbat_fg = %duV, fv = %duV\n",
						prop.intval, effective_fv_uv);
		goto check_done;
	}

	if(iterm_fv_uv <=  (effective_fv_uv)){
		iterm_cap_mAh = qg_get_near_cap_from_vbat(iterm_fv_uv);
	}

	if(VBAT_MAX_VOL_UV >= effective_fv_uv){
		pr_err("Maybe jeita or long charge mode was enabled  %d mV",effective_fv_uv);
	}
...
#ifdef CONFIG_HISENSE_CHARGE_FG_FUNCTION
#define DEFAULT_SHUTDOWN_SOC_SECS	3600	/*1 hour*/
#else /*CONFIG_HISENSE_CHARGE_FG_FUNCTION*/
...
static int qpnp_qg_probe(struct platform_device *pdev)
{
	...
	pr_buf_info("QG initialized! battery_profile=%s SOC=%d QG_subtype=%d QG_version=%s QG_mode=%s\n",
