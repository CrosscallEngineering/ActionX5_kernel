...
static struct pil_tz_data *modem_drv;

void exec_restart_modem_ctrl(void)
{
	if (!modem_drv)
		pr_err("restart modem failed and modem_drv is null\n");
	else {
		pr_info("restart modem subsys name is %s\n", modem_drv->subsys_desc.name);
		log_failure_reason(modem_drv);
		subsystem_restart_dev(modem_drv->subsys);
	}
}
#endif /* CONFIG_HISENSE_RESTART_MODEM */
...
static int pil_tz_driver_probe(struct platform_device *pdev)
{
	...
	if (d->pas_id == PAS_MODEM_SW) {
		modem_drv = d;
		dev_info(&pdev->dev, "Get Subsys:%s handle for Reset\n", d->subsys_desc.name);
	}
#endif
