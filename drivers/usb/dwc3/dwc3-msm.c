...
extern bool sgm2540_af_otg_ok(void);
extern bool sgm2540_bf_otg_ok(void);
...
#ifdef CONFIG_HISENSE_SGM2540
		if (!sgm2540_af_otg_ok() && !sgm2540_bf_otg_ok()) {
			ret = sgm2540_b_otg_vbus_enable(true);
			dev_err(mdwc->dev, "sgm2540_b_otg_vbus enable, ret = %d\n", ret);
		}
#endif /*CONFIG_HISENSE_SGM2540*/
...
#ifdef CONFIG_HISENSE_SGM2540
		if (sgm2540_bf_otg_ok()) {
			ret = sgm2540_b_otg_vbus_enable(false);
			dev_err(mdwc->dev, "sgm2540_b_otg_vbus disable, ret = %d\n", ret);
		} else if (sgm2540_af_otg_ok()) {
			//add disable magconn otg to solve af irq trigger because of vbus not draw down.
			ret = sgm2540_a_otg_vbus_enable(false);
			dev_err(mdwc->dev, "sgm2540_a_otg_vbus disable, ret = %d\n", ret);
		}
#endif /*CONFIG_HISENSE_SGM2540*/

#ifdef CONFIG_HISENSE_SGM41512_CHARGER
		if (sgm_typec_otg_ok()) {
			ret = sgm_typec_otg_usb_switch(false);
			dev_err(mdwc->dev, "sgm_typec_otg_usb_switch disable, ret = %d\n", ret);
		}
#endif /*CONFIG_HISENSE_SGM41512_CHARGER*/
