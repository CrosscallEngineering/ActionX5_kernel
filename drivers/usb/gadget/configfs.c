		...
		const char *name)
{
	...
	if (!instance_name) {
		pr_buf_err("Unable to locate . in FUNC.INSTANCE\n");
		return ERR_PTR(-EINVAL);
	}
	*instance_name = '\0';
	instance_name++;
		...
		const char *name)
{
	...
	num_str = strchr(buf, '.');
	if (!num_str) {
		pr_buf_err("Unable to locate . in name.bConfigurationValue\n");
		return ERR_PTR(-EINVAL);
	}
		...
		struct usb_gadget_driver *gdriver)
{
	...
	if (list_empty(&gi->cdev.configs)) {
		pr_buf_err("Need at least one configuration in %s.\n",
				gi->composite.name);
	...
	list_for_each_entry(c, &gi->cdev.configs, list) {
		...
		if (list_empty(&cfg->func_list)) {
			pr_buf_err("Config %s/%d of %s needs at least one function.\n",
...
static void android_work(struct work_struct *data)
{
	...
	if (!uevent_sent) {
		pr_buf_info("%s: did not send uevent (%d %d %pK)\n", __func__,
