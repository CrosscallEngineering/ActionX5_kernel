				...
				struct usb_endpoint_descriptor *intr_desc)
{
	...
	pr_err("usb_ep_autoconfig for ep_in got %s\n", ep->name);
	...
	pr_err("usb_ep_autoconfig for mtp ep_out got %s\n", ep->name);
	ep->driver_data = dev;		/* claim the endpoint */
	dev->ep_out = ep;

	ep = usb_ep_autoconfig(cdev->gadget, intr_desc);
	if (!ep) {
		pr_err("usb_ep_autoconfig for ep_intr failed\n");
		return -ENODEV;
	}
	pr_err("usb_ep_autoconfig for mtp ep_intr got %s\n", ep->name);
	ep->driver_data = dev;		/* claim the endpoint */
	dev->ep_intr = ep;
	...
	while (count > 0 || sendZLP) {
		...
		ret = wait_event_interruptible(dev->write_wq,
			((req = mtp_req_get(dev, &dev->tx_idle))
				|| dev->state != STATE_BUSY));
		if (!req) {
			pr_err("request NULL ret:%d state:%d\n",
				ret, dev->state);
			r = ret;
			break;
		}
...
static void receive_file_work(struct work_struct *data)
{
	...
	if (!IS_ALIGNED(count, dev->ep_out->maxpacket))
		pr_err("- count(%lld) not multiple of mtu(%d)\n",
	...
	struct mtp_file_range *mfr)
{
	...
	if (mtp_lock(&dev->ioctl_excl)) {
		pr_err("ioctl returning EBUSY state:%d\n", dev->state);
		return -EBUSY;
	}
	...
	unsigned long value)
{
	...
	case COMPAT_MTP_SEND_EVENT:
		...
		break;
	default:
		pr_err("unknown compat_ioctl code: %d\n", code);
		ret = -ENOIOCTLCMD;
		goto fail;
	}
				...
				const struct usb_ctrlrequest *ctrl)
{
	...
	if (ctrl->bRequestType ==
		...
		pr_err("class request: %d index: %d value: %d length: %d\n",
			ctrl->bRequest, w_index, w_value, w_length);

		if (ctrl->bRequest == MTP_REQ_CANCEL && w_index == 0
				&& w_value == 0) {
			pr_err("MTP_REQ_CANCEL\n");
			...
			pr_err("MTP_REQ_GET_DEVICE_STATUS\n");
			spin_lock_irqsave(&dev->lock, flags);
