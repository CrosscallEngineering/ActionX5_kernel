...
#define FS_SYNC_INTERVAL_MS			2000
#define FS_SYNC_MAX_TIMERS 			6  //Final sync time FS_SYNC_INTERVAL_MS*FS_SYNC_MAX_TIMERS
...
static void fs_sync_func(struct work_struct *work)
{
	...
	ksys_sync();
	dev_info(pon->dev, "sys_sync:fs_sync_func_finish times:%d\n", pon->fs_sync_counter);
...
void hs_set_pm_pon_resin(int debug)
{
	...
	/* Turn off KPDPWR+RESIN warm reset function in Factory mode */
	if(dev_bi.bootmode == BOOT_FACTORY_MODE)
	    qpnp_config_reset_for_hs(PON_KPDPWR_RESIN, 0, 0, 0, 0);
}
