...
int cts_irq_handler(struct cts_device *cts_dev)
{
    ...
    if (unlikely(cts_dev->rtdata.suspended)) {
        ...
        if (cts_dev->rtdata.gesture_wakeup_enabled) {
            ...
            if (ret) {
                cts_err("send gesture suspend cmd failed %d", ret);
            }
...
#ifdef CONFIG_CHIPONE_TDDI_GLOVE
	if (cts_dev->rtdata.glove_enable) {
		cts_enter_glove_mode(cts_dev);
	}
#endif /* CONFIG_CHIPONE_TDDI_GLOVE */
    return 0;
...
#endif /* CONFIG_TOUCHSCREEN_CHIPONE_TDDI_GLOVE */
...
int cts_fw_enable_monitor_mode(const struct cts_device *cts_dev)
{
	int ret;
	u8 value;

	ret = cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_MONITOR_MODE, &value);
	if (ret)
		return ret;

	if ((value & BIT(0)) == 0) {
		return cts_fw_reg_writeb(cts_dev, CTS_DEVICE_FW_REG_MONITOR_MODE,
				value | BIT(0));
	}
...
int cts_fw_disable_monitor_mode(const struct cts_device *cts_dev)
{
	int ret;
	u8 value;
...
int cts_set_monitor_mode_count(
		const struct cts_device *cts_dev, u8 count)
{
	int ret;

	ret = cts_fw_reg_writeb(cts_dev, 0x8000 + 219, count);
	if (ret)
		return ret;
