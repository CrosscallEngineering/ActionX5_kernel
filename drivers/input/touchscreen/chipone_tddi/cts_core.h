...
enum cts_device_fw_reg {
	...
	CTS_DEVICE_FW_REG_MODULE_ID = 0x0044,
	...
	CTS_DEVICE_FW_REG_MONITOR_MODE = 0x8000 + 344,
...
#endif
extern int firmware_builtin_version;
...
extern int cts_fw_enable_monitor_mode(const struct cts_device *cts_dev);
extern int cts_fw_disable_monitor_mode(const struct cts_device *cts_dev);
extern int cts_set_monitor_mode_count(const struct cts_device *cts_dev, u8 count);
#else
static inline int cts_fw_enable_monitor_mode(const struct cts_device *cts_dev) {return 0;}
static inline int cts_fw_disable_monitor_mode(const struct cts_device *cts_dev) {return 0;}
static inline int cts_set_monitor_mode_count(const struct cts_device *cts_dev, u8 count) {return 0;}
#endif
#ifdef CONFIG_CTS_CHARGER_DETECT
