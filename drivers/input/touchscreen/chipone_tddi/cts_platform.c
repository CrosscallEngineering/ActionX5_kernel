...
int cts_spi_send_recv(struct cts_platform_data *pdata, size_t len , u8 *tx_buffer, u8 *rx_buffer)
{
    ...
    buf = (u8 *)kzalloc(len, GFP_KERNEL | GFP_DMA);
    memcpy(buf, tx_buffer, len);
    cmd.tx_buf = buf;
...
static irqreturn_t cts_plat_irq_handler(int irq, void *dev_id)
{
	...
	if(pdata->cts_sys_suspend)
		__pm_wakeup_event(pdata->cts_wakelock, 500); //500ms
...
static int cts_plat_irq_thread_fn(void *arg)
{
    ...
    do {
        ...
        wait_event_interruptible(pdata->irq_waitq_head,
            ((pdata->irq_pending != 0) && (pdata->cts_sys_suspend == false)));
        pdata->irq_pending = 0;
        ...
        struct device_node *dev_node)
{
	...
	ret = of_property_read_string(dev_node, "chipone,factory-info",
			&pdata->factory_info);
	if (ret) {
		cts_err("factory info not support, set to NULL");
		pdata->factory_info = "NULL";
	}

#ifdef CONFIG_TOUCHSCREEN_CHIPONE_TDDI_GESTURE
	prop = of_find_property(dev_node, "chipone,gesture-func-map", NULL);
	if (prop) {
		num_buttons = prop->length / sizeof(temp_val);
		if (num_buttons > CFG_CTS_NUM_GESTURE)
			return -EINVAL;

		rc = of_property_read_u32_array(dev_node,
			"chipone,gesture-func-map", &pdata->gesture_keymap[1][0],
			num_buttons);
		if (rc) {
			cts_err("Unable to read gesture func map");
			return rc;
		}
		pdata->gesture_num = num_buttons;
	}

	prop = of_find_property(dev_node, "chipone,gesture-figure-map", NULL);
	if (prop) {
		rc = of_property_read_u32_array(dev_node,
			"chipone,gesture-figure-map", &pdata->gesture_keymap[0][0],
			num_buttons);
		if (rc) {
			cts_err("Unable to read gesture figure map");
			return rc;
		}
	}
#endif
    ...
    struct cts_device_gesture_info *gesture_info)
{
	...
	for (i = 0; i < CFG_CTS_NUM_GESTURE; i++) {
		if (gesture_info->gesture_id == pdata->gesture_keymap[0][i]) {
			struct chipone_ts_data *cts_data;

			cts_data = container_of(pdata->cts_dev, struct chipone_ts_data, cts_dev);

			if ((cts_data->gesture_state & BIT(i + 1)) != 0) {
				cts_info("Report key[%u]", pdata->gesture_keymap[1][i]);
				input_report_key(pdata->ts_input_dev, pdata->gesture_keymap[1][i], 1);
				input_sync(pdata->ts_input_dev);
				input_report_key(pdata->ts_input_dev, pdata->gesture_keymap[1][i], 0);
				input_sync(pdata->ts_input_dev);
			}
	...
	cts_warn("Process unrecognized gesture id = 0x%02x", gesture_info->gesture_id);

    return -EINVAL;
}

#endif /* CONFIG_TOUCHSCREEN_CHIPONE_TDDI_GESTURE */
