...
extern struct spi_device *CTS_Client;
...
#ifdef CONFIG_TOUCHSCREEN_CHIPONE_TDDI_GLOVE
...
int cts_test_rdy_flag(struct cts_device *cts_dev)
{
    int i, j;
...
static int validate_test_param (const struct cts_test_param *param)
{
    ...
    if (!!(param->flags & CTS_TEST_FLAG_VALIDATE_DATA)) {
        if ((!!(param->flags & CTS_TEST_FLAG_VALIDATE_MIN)) &&
            param->min == NULL) {
            return -EINVAL;
        }
        if ((!!(param->flags & CTS_TEST_FLAG_VALIDATE_MAX)) &&
            param->max == NULL) {
            return -EINVAL;
        }
    }

    if (!!(param->flags & CTS_TEST_FLAG_DUMP_TEST_DATA_TO_FILE)) {
        if (param->test_data_filepath == NULL) {
            return -EINVAL;
        }
    }

    if (!!(param->flags & CTS_TEST_FLAG_DUMP_TEST_DATA_TO_USERSPACE)) {
        if (param->test_data_buf == NULL ||
            param->test_data_wr_size == NULL) {
            return -EINVAL;
        }
    }

    if (!!(param->flags & CTS_TEST_FLAG_DRIVER_LOG_TO_FILE)) {
        if (param->driver_log_filepath == NULL) {
            return -EINVAL;
        }
    }

    if (!!(param->flags & CTS_TEST_FLAG_DRIVER_LOG_TO_USERSPACE)) {
        if (param->driver_log_buf == NULL ||
            param->driver_log_wr_size == NULL) {
            return -EINVAL;
        }
    }
...
static int factory_proc_fw_update(struct device *dev, bool force)
{
	...
	if (!cts_data->cts_dev.rtdata.updating) {
		...
		if (firmware == NULL) {
			cts_err("Request firmware from %s failed",
				firmware_name);
		...
		if (force ||
			(FIRMWARE_VERSION(firmware) > cts_data->cts_dev.fwdata.version)) {
			cts_stop_device(&cts_data->cts_dev);
			cts_update_firmware_from_file(&cts_data->cts_dev, firmware_name, true);
...
static int factory_proc_fw_bin_update(struct device  *dev, const char *buf)
{
	...
	if (!cts_data->cts_dev.rtdata.updating) {
		...
		cts_start_device(&cts_data->cts_dev);
		if (ret < 0) {
			dev_err(&cts_data->spi_client->dev, "%s:upgrade failed.\n", __func__);
		}
	}
	mutex_unlock(&cts_data->cts_dev.pdata->ts_input_dev->mutex);
	cts_ts_register_productinfo(cts_data);
...
static int factory_proc_hibernate_test(struct device *dev)
{
	...
	if (err) {
		dev_err(dev, "%s: Chipone_tddi reset test failed.\n", __func__);
		return 0;
	}else{
		printk("%s Chipone_tddi reset test success!\n", __func__);
		return 1;
	}
}
...
#if defined(CONFIG_TOUCHSCREEN_CHIPONE_TDDI_GESTURE)
...
static int cts_set_erase_flash_test(struct device *dev,bool enable)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	struct cts_device *cts_dev = &cts_data->cts_dev;
	int ret = 0;
	...
	if (enable) {
		...
		ret = cts_erase_flash(cts_dev, 0, 0x20000);
		if (ret) {
			cts_err("Chipone_tddi erase flash success");
		}
	}
	return ret;
}

#ifdef CONFIG_TOUCHSCREEN_CHIPONE_TDDI_GESTURE
...
#endif
#if defined(CONFIG_TOUCHSCREEN_CHIPONE_TDDI_INCELL_CHIP)
static int cts_ts_suspend_for_lcd_async_use(void)
{
	struct chipone_ts_data *cts_data;
	if (CTS_Client == NULL)
		return -ENODEV;

	cts_data = dev_get_drvdata(&CTS_Client->dev);

	cancel_work_sync(&cts_data->resume_work);
	return cts_suspend(cts_data);
}

static int cts_ts_resume_for_lcd_async_use(void)
{
	struct chipone_ts_data *cts_data;
	if (CTS_Client == NULL)
		return -ENODEV;
...
static int cts_ts_provide_reset_control(void)
{
	struct chipone_ts_data *cts_data;
	cts_data = dev_get_drvdata(&CTS_Client->dev);

	if ((cts_data == NULL) || (cts_data->spi_client== NULL))
		return -ENODEV;
...
#endif
#if defined(CONFIG_TOUCHSCREEN_CHIPONE_TDDI_INCELL_CHIP) && defined(CONFIG_TOUCHSCREEN_CHIPONE_TDDI_GESTURE)
int cts_need_lcd_power_reset_keep_flag_get(void)
{
	struct chipone_ts_data *cts_data;
	cts_data = dev_get_drvdata(&CTS_Client->dev);
...
int factory_ts_func_test_register(struct chipone_ts_data* data)
{
	...
	data->ts_test_dev.dev = &data->spi_client->dev;
...
#if defined(CONFIG_TOUCHSCREEN_CHIPONE_TDDI_GLOVE)
