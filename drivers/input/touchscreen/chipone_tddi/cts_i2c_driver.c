...
extern int factory_ts_func_test_register(struct chipone_ts_data* data);
extern void cts_ts_register_productinfo(struct chipone_ts_data *cts_data);
struct spi_device *CTS_Client = NULL;
...
int cts_suspend(struct chipone_ts_data *cts_data)
{
	...
	if (!cts_is_device_enabled(&cts_data->cts_dev)) {
		cts_info("cts_suspend while already suspend");
		return 0;
	}
...
static int cts_resume(struct chipone_ts_data *cts_data)
{
    ...
    cts_info("Resume");
	if (cts_is_device_enabled(&cts_data->cts_dev)) {
		cts_info("cts_resume while already resume");
		return 0;
	}
...
static void cts_ts_resume_work(struct work_struct *work)
{
	struct chipone_ts_data *ts_data = container_of(work,
			struct chipone_ts_data, resume_work);
...
#endif
{
	...
	cts_data->buildin_fw_version = firmware_builtin_version;
	...
	INIT_WORK(&cts_data->resume_work, cts_ts_resume_work);
#ifdef CONFIG_CTS_PM_FB_NOTIFIER
...
#ifdef CONFIG_TOUCHSCREEN_CHIPONE_TDDI_GESTURE
	cts_data->gesture_state = 0;
	cts_data->cts_dev.rtdata.gesture_wakeup_enabled = false;
#endif
	CTS_Client = cts_data->spi_client;

	factory_ts_func_test_register(cts_data);
	cts_ts_register_productinfo(cts_data);
	wakeup_source_register(cts_data->device, "cts-wakelock");
	cts_data->pdata->cts_sys_suspend = false;
	cts_info("cts_driver_probe exit.");
    return 0;
...
static int cts_driver_pm_suspend(struct device *dev)
{
...
static int cts_driver_pm_resume(struct device *dev)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
...
static const struct dev_pm_ops cts_driver_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(cts_driver_pm_suspend,
		cts_driver_pm_resume)
};
...
{
    return scnprintf(buf, PAGE_SIZE, "CONFIG_TOUCHSCREEN_CHIPONE_TDDI_GESTURE: %c\n",
#ifdef CONFIG_TOUCHSCREEN_CHIPONE_TDDI_GESTURE
        'Y'
#else
        'N'
...
static int __init cts_driver_init(void)
{
	...
	if(ret != 0)
		cts_err("spi_register_driver failed");
	cts_info("spi_register_driver...");
