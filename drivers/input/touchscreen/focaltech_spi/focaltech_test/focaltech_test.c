...
static struct proc_dir_entry *tp_differ_proc = NULL;
u8 tx_num = 0;
u8 rx_num = 0;
...
#endif
extern int fts_boot_write(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen);
...
#ifdef CONFIG_TOUCHSCREEN_FT8006P_SPI_ESDCHECK
...
unsigned char ReadDiffData(unsigned char Freq, unsigned char LineNum, int ByteNum, short *pRevBuffer)
{
    ...
    ReCode = fts_write_reg(Freq, LineNum);//Set row addr;
    ...
    //***********************************************************Read differ data
    I2C_wBuffer[0] = FACTORY_REG_RAWDATA_ADDR;   //set begin address
    if (ReCode == ERROR_CODE_OK) {
        sys_delay(10);
        ReCode = fts_read(I2C_wBuffer, 1,m_ucdiffData, tx_num*rx_num*2);
        if (ReCode != ERROR_CODE_OK) {
...
static unsigned char Getdiff(void) {
    ...
    ReCode = fts_read_reg(0x06, &orgvalue); // read orginal value
    FTS_INFO("0x06 orgvalue = %d", orgvalue);
    ReCode = fts_write_reg(0x06, 0x01);// switch to read diff
    if (ReCode != ERROR_CODE_OK) {
        FTS_TEST_ERROR("\nWriteReg error. \n");
        return ReCode;
    }
    ...
    ReCode = ReadDiffData( 1, 0xAD, tx_num*rx_num*2, m_iTempDiffData );
   ...
   ReCode = fts_write_reg(0x06,orgvalue);
   if (ReCode != ERROR_CODE_OK) {
...
static int tp_differ_proc_show(struct seq_file *file, void* data)
{
    unsigned char ReCode = ERROR_CODE_COMM_ERROR;
    int iRow, iCol;
    int ret = 0;
    ...
    fts_read_reg(0x02,&tx_num);
    fts_read_reg(0x03,&rx_num);
    ...
    if (ReCode == ERROR_CODE_OK) {
        ...
        FTS_TEST_ERROR("cat proc tp_differ %d\n", ReCode);
        goto TEST_ERR;
    }
...
int factory_get_diff(struct device *dev, char *buf)
{
    ...
    if (ReCode == ERROR_CODE_OK) {
        for (iRow = 0; iRow < tx_num; iRow++) {
            for (iCol = 0; iCol < rx_num; iCol++) {
				count += snprintf(buf + count, PAGE_SIZE, "%5d ", m_diffdata[iRow][iCol]);
            }
...
int fts_test_init(struct fts_ts_data *ts_data)
{
	...
	if (ret < 0) {
        ...
        if (ret) {
            FTS_TEST_ERROR( "ERROR: create fts_tp_differ proc()  failed.");
        }
#endif
