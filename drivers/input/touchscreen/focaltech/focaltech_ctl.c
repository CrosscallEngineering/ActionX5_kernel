...
static void fts_rw_iic_drv_setup_cdev(struct fts_rw_i2c_dev *dev, int index)
{
	...
	if (err)
		pr_buf_err("Error %d adding LED%d", err, index);
}
...
static int fts_rw_iic_drv_myinitdev(struct i2c_client *client)
{
	...
	if (err < 0) {
		pr_buf_err("%s:ft_rw_iic_drv failed  error code=%d---\n", __func__, err);
		return err;
	}
	...
	if (!fts_rw_i2c_dev_tt) {
		...
		pr_buf_err("%s:ft_rw_iic_drv failed\n", __func__);
		return err;
	}
	...
	if (IS_ERR(fts_class)) {
		pr_buf_err("%s:failed in creating class.\n", __func__);
		return -ENODEV;
	}
	/*create device node*/
...
{
	pr_buf_info("[FTS]----ft_rw_iic_drv init ---\n");
