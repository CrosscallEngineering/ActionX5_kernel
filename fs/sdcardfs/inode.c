...
#if defined(CONFIG_SDCARD_FS_BYPASS_PERM_CHECK)
	perm = top->perm;
	app_uid = from_kuid(&init_user_ns, current_fsuid());
	if (err == -EACCES
			&& (perm >= PERM_ANDROID_DATA && perm <= PERM_ANDROID_PACKAGE_CACHE)
			&& is_bypass_uid(app_uid))
	{
		//pr_info("sdcardfs: sdcardfs_permission err=%d app_uid=%d perm=%d under_android=%d under_cache=%d under_obb=%d\n",
		//		err, app_uid, perm, top->under_android, top->under_cache, top->under_obb);
		err = 0;
	}
#endif
