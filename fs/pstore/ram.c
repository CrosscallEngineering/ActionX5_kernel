...
static int ramoops_probe(struct platform_device *pdev)
{
	...
	pr_info("pdata->mem_size=0x%x record_size=0x%x console_size=0x%x ftrace_size=0x%x pmsg_size=0x%x\n",
...
{
	int ret;
	pr_info("ramoops_init\n");

	ramoops_register_dummy();
	ret = platform_driver_register(&ramoops_driver);
	if (ret != 0) {
		pr_info("ramoops_driver failed\n");
		ramoops_unregister_dummy();
		}
